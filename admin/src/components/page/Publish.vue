  ...
  <div>
    ...
    <div class="plugins-tips">
      <p>这个时候就需要解析markdown语法了。可以使用
        <a href="https://github.com/miaolz123/vue-markdown" target="_blank">vue-markdown</a>：一个基于vue.js的markdown语法解析插件。（这里不作展开，有需要自行了解）
      </p>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    getAllCate() {
      let that = this;
      this.$axios.get(this.$API.Cate.getAll).then(res => {
    ...
    edit() {
      ...
      this.$axios
        .post(this.$API.Article.update, this.article)
        .then(res => {
          if (res.data.data == 1) {
            //更新成功 跳转到文章
    ...
    publish() {
      ...
      this.article.cate = this.article.cateName;
      this.$axios
        .post(this.$API.Article.publish, this.article)
        .then(res => {
